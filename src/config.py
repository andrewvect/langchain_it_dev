...
HUMAN_IN_THE_LOOP = os.environ.get("HUMAN_IN_THE_LOOP", "True").lower() not in ("false", "0", "no")
...
def get_llm_api_key() -> str:
    ...
    if isinstance(key, dict):
        key = key.get("api_key")
    if not key:
        raise ValueError(f"API key for provider '{LLM_PROVIDER}' not found in secrets.json")
    return str(key)
